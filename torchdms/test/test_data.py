...
def test_partition_is_clean():
    """
    Ensure that the test and train partitions have no shared variants on the amino acid
    level, over 50 random seeds.
    """
    data, _ = from_pickle_file(TEST_DATA_PATH)
    for seed in range(50):
        random.seed(seed)
        (test, partitioned_train) = partition(
            data,
            per_stratum_variants_for_test=15,
